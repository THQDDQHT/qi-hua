...
- 公众初版界面：电脑端访问根地址或点击“啟画”应显示海报首页，点击“开始使用”进入生图工作台；手机端访问根地址或点击“啟画”仍直接进入生图工作台。桌面导航只显示生图工作台、提示词库和我的素材，手机底部只显示生图与素材；画布、视频、配置、Codex、GitHub、文档和版本入口均不显示，且公众模式不能访问画布、视频与配置路由。
- 公众生图参数：质量、画面比例和生成数量由前端固定为按钮式面板，显示自动/高/中/低、当前完整尺寸选项、受控自定义宽高和每次 1–10 张；这些参数均应通过服务端校验。会话接口不可用时选项仍应显示，但生成按钮保持禁用。
- 公众本地数据提示：首次访问和后续使用均不再显示本地备份弹窗或页面底部提示横幅。

- 品牌名称：Web 浏览器标题、PWA 安装信息、顶部导航、首页、画布库、默认项目名、导出与本地备份文件名、文档站、本地 Agent 和 Codex 插件应统一显示“啟画”；同时验证旧本地数据和旧 ZIP 仍可读取，`infinite-canvas` 存储键、导出格式、MCP 命令和 Service Worker 缓存标识保持兼容。

- 公众生图 PostgreSQL 并发边界：验证同一设备并发成功数每天最多 10 张、同一来源地址并发成功数每天最多 30 张，并复核幂等、部分成功和回收竞态。状态：BLOCKED（当前环境无 `TEST_DATABASE_URL`；新增 CI 将运行现有 PostgreSQL 集成测试，但生产压力与权限仍需部署环境验证）。
- 公众生图真实 API：使用受控供应商测试账号验证成功、401/403、429、5xx、超时、空图和非法图响应，以及额度结算与日志脱敏。状态：BLOCKED（当前无真实供应商测试凭据，自动化只使用本地 fake provider）。
- 公众生图断连与迟到结果：验证客户端在请求中途断开后服务端仍完成结算，超时或过期后的迟到图片不交付。状态：BLOCKED（需要真实 HTTP 连接和 PostgreSQL 环境）。
- 公众生图重启恢复：验证 api 进程或容器在 `reserved`/`running` 状态重启后，启动回收和每 60 秒调度能释放过期预占且不重复扣减。状态：BLOCKED（当前无 Docker 和 PostgreSQL 运行环境）。
- 公众生图容器与 Nginx：验证双容器真实启动、仅 API 加入既有 PostgreSQL 外部网络、可信代理与局域网直连来源地址、`/health/live`/`ready`、反代、22MB 请求限制、190 秒超时、5r/m + burst 2 限流、缓存和 CSP。状态：BLOCKED（CI 只构建镜像、展开 Compose 并运行 `nginx -t`，不等同运行时验收）。
- 公众生图 Cloudflare 与源站：在测试域名验证 Full (strict)、可信 `CF-Connecting-IP` 恢复、源站 80/443 仅允许 Cloudflare 网段、源站 IP 不可绕过，以及关闭开关止损。状态：BLOCKED（需要 Cloudflare、DNS、证书和防火墙权限）。
- 公众生图移动与桌面交互：在 iPhone Safari、Android Chrome 验证相册/拍照、生成、预览、下载、素材和 PWA 安装。状态：BLOCKED（当前无真机和可自动化截图的显示环境）。
