...
- 公众生图容器与 Nginx：验证双容器真实启动、仅 API 加入既有 PostgreSQL 外部网络、`/health/live`/`ready`、反代、22MB 请求限制、190 秒超时、5r/m + burst 2 限流、缓存和 CSP。状态：BLOCKED（CI 只构建镜像、展开 Compose 并运行 `nginx -t`，不等同运行时验收）。
