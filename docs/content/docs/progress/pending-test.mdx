...
- 公众生图部署：新增独立双容器 Compose、Cloudflare 可信地址 include、Nginx 反代/限流/缓存/CSP 配置及迁移、smoke、配置和日志安全脚本；需在受 Cloudflare 严格 TLS 和源站防火墙保护的测试域名上，验证 `/health/ready`、非生图 smoke、图片接口 5r/m 限流、源站不可绕过和关闭开关止损。
